...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    let viewName = "type"
    let postIdentifier = "id"
    let paramToken = "push_id"
    ...
    func tokenRefreshNotification(_ notification: Notification) {
        if let refreshedToken = FIRInstanceID.instanceID().token() {
            ...
            registerDevice(token: refreshedToken)
        }
    ...
    func registerDevice(token: String){
        ...
        if !defaults.bool(forKey: flag){
            ...
            Alamofire.request(linkSaveToken, method: .post, parameters: params).responseJSON{
                response in
